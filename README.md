Domain
========
<!--
|<- - - - - - - This line is exactly 80 characters for reference - - - - - - ->|
-->
This is currently a beta release. We are finding that the module generally seems 
to work, but it needs more testing before we can assume it's stabile. Please, 
test it out and report bugs. But, be cautious before installing in a 
production environment. 

There were many Drupal 7 modules to extend and improve this module. Please, see
a list of other domain related modules that have been ported to Backdrop or 
developed specifically for Backdrop. Please, help us port more modules 
to extend and support the domain access module.

https://github.com/backdrop-contrib/domain/wiki/1.-Domain-module-wiki

Documentation
-------------------
Please review the documentation in the wiki and help us make it better. 
https://github.com/backdrop-contrib/domain/wiki


Installation
------------
- Install this module using the official Backdrop CMS instructions at
  https://docs.backdropcms.org/documentation/extend-with-modules.
- This module requires modifications to the settings.php file. Be sure to review:
  https://github.com/backdrop-contrib/domain/wiki/2.-Domain-Quick-Start-Install

Issues
------
Bugs and Feature Requests should be reported in the Issue Queue:
https://github.com/backdrop-contrib/domain/issues.

Current Maintainers
-------------------
- [Tim Erickson](https://github.com/stpaultim) - [Simplo by Triplo](https://simplo.site/)
- [Rodobaldo Perez Macias](https://github.com/rudy880719)
- Collaboration and co-maintainers welcome! REALLY, REALLY WELCOME!!

Credits
-------

Port to Backdrop CMS by:
- [Rodobaldo Perez Macias](https://github.com/rudy880719)
- [Martin Price](https://github.com/yorkshire-pudding) - [System Horizons Ltd](https://www.systemhorizons.co.uk)
- [Tim Erickson](https://github.com/stpaultim) - [Simplo by Triplo](https://simplo.site/)
- Could you help? Help is needed to document issues, work on fixes and to test fixes.

Drupal maintainers:
- [Ken Rickard (agentrickard)](https://www.drupal.org/u/agentrickard)
- [Katrin Valdre (nonsie)](https://www.drupal.org/u/nonsie)

Drupal development sponsored by [Palantir.net](https://palantir.net/)

Created for Drupal by [Gurpartap Singh](https://www.drupal.org/u/gurpartap-singh)

License
-------

This project is GPL v2 software.
See the LICENSE.txt file in this directory for complete text.
